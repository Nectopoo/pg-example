...
public class SmartReplicationConfig {
    ...
    public AbstractCachedResolver changeSourceResolver(IConfigurationManagerPrivate configManager,
                                                       ...
                                                       KafkaProducerHolder kafkaProducerHolder) {
        final WatcherChangeSourceResolver watcherChangeSourceResolver = new WatcherChangeSourceResolver(new DistributedChangeSourceResolver(conf.getOwner(), configManager),
            customReplicationPropertiesProvider::getDefaultChangeSource,
            customReplicationPropertiesProvider::isReplicationEnabled);
        watcherChangeSourceResolver.addListener(kafkaProducerHolder);
...
//        sslMapConfig.put(SslConfigs.SSL_KEYSTORE_PASSWORD_CONFIG, "password");
    ...
    @Bean
    public KafkaProducerHolder kafkaProducerHolder(KafkaProducerConfig kafkaProducerConfig) {
        return new KafkaProducerHolder(kafkaProducerConfig);
    }

    @Bean
    public KafkaChangeHandler kafkaChangeHandler(KafkaProducerHolder kafkaProducerHolder) {
...
//        return new EmptyCustomReplicationPropertiesProvider();
     ...
     */
    @Bean
    public EmergencyFailureHandler<Change> emergencyFailureHandler(EnableReplicationResolver changeSourceResolver,
                                                                   CustomReplicationPropertiesProvider customReplicationPropertiesProvider) {
...
//                conf.getMaxReplicationErrors(), conf.getErrorPeriodInSeconds());
        return new MyEmergencyFailureHandler(changeSourceResolver, customReplicationPropertiesProvider,
                conf.getMaxReplicationErrors(), conf.getErrorPeriodInSeconds());
    }
